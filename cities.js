...
const findLat = citiesListBase => (city) => R.pipe(
    R.filter(R.propEq('city', city)),
    R.head(),
    R.path(['lat'])
)(citiesListBase);

const findLong = citiesListBase => (city) => R.pipe(
    R.filter(R.propEq('city', city)),
    R.head(),
    R.path(['long'])
)(citiesListBase);
...
const DistanceBetweenTwoCities = (city_1, city_2) =>
    distHaversine(
        findLat(citiesListBase)(city_1),
        findLong(citiesListBase)(city_1),
        findLat(citiesListBase)(city_2),
        findLong(citiesListBase)(city_2));

const AllCitiesName = R.pipe(R.pluck('city'))(citiesListBase);
        ...
        if (R.equals(R.length(list) % (R.length(AllCitiesNameFirst)-1), (R.length(AllCitiesNameFirst)-2)) == true) {
            return Loop(
                ...
                AddNewCity(list)(
                    ...
                    findLat(citiesListBase)(cityFrom),
                    findLong(citiesListBase)(cityFrom)
                ),
            );
        }
        else {
